...
def with_source_prefix(reports, prefix):
  def second_line(r):
    a = r.find('\n') + 1;
    b = r.find('\n', a)
    return r[a:b]
  ...
  return [r for r in reports if source_name(r).startswith(prefix)]
...
def main(*argv, **kwargs):
    ...
    gcov.add_argument('--gcov-source-prefix', default='', help="Check gcov files for :Source:$prefix")
    ...
    try:
        ...
        if codecov.gcov_source_prefix:
          reports = with_source_prefix(reports, codecov.gcov_source_prefix)
